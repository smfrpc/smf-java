...
class StorageService implements RpcService {
  ...
  private final Function<byte[], byte[]> RESPONSE_HANDLER = (request) -> {
    ...
    LOG.info("[{}] Handling incoming request ! plain : {}", currentThreadName,
             new String(request));
    int responsePosition =
      responseBuilder.createString("RESPONSE FROM JAVA-SERVER! ");
  ...
  getHandler(long id) {
    if (id == GET_METHOD_META) { return RESPONSE_HANDLER; }
