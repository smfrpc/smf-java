...
import smf.client.core.exceptions.SessionIdGenerationException;

/**
 * Generate sessionId with max to {@param USHRT_MAX} because of SMF sessionsId
 * max value restrictions. SessionIdGenerator is Thread Safe and can be used
...
public class SessionIdGenerator {
  ...
  private final ConcurrentHashMap<Integer, Boolean> pendingSessions =
    new ConcurrentHashMap<>();
  private final AtomicInteger sessionIdGen = new AtomicInteger(0);

  /**
   * Generate new sessionId not used by any pending request. Method is thread
   * safe and doesn't block. Internally sessionId is assigned in "compare and
   * set" manner, and if this assignment will fail {@param MAX_RETRIES} times,
   * SessionIdGenerationException exception will be thrown.
   *
   * @return sessionId that can be used by RPC request.
   * @throws SessionIdGenerationException if method cannot generate sessionId
  ...
  next() {
    ...
    while (true) {
      ...
      /**
       * Even if sessionIdGen will turn over and two threads will get the same
       * number, putIfAbsent is atomic, and one of those threads will fail.
       */
      ...
      if (iteration > MAX_RETRIES) {
        throw new SessionIdGenerationException(
          "Cannot generate sessionId because of "
          + "height contention with other threads ! " + MAX_RETRIES +
          " performed");
      }
