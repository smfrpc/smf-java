...
public class CompressionService {
  ...
  private static final LZ4Factory blazingFastFactory =
  ...
  private byte[] compressUsingZstd(byte[] body) {
    if (LOG.isDebugEnabled()) { LOG.debug("Compressing using ZSTD"); }
  ...
  private byte[] decompressUsingZstd(byte[] body) {
    if (LOG.isDebugEnabled()) { LOG.debug("Decompressing using ZSTD"); }
  ...
  private byte[] decompressUsingLz4(byte[] body) {
    ...
    /**
     * I have doubt about this int-long exchangeability (because SMF c++ uses
     * unsigned-int), but, even if we will receive more than max Java's INT, it
     * will not be able to handle such bit array :/
     */
  ...
  private byte[] compressUsingLz4(byte[] body) {
    if (LOG.isDebugEnabled()) { LOG.debug("Compressing using LZ4"); }

    int maxCompressedLength = lz4Compressor.maxCompressedLength(body.length);
    byte[] compressed = new byte[maxCompressedLength];
    int compressedLength = lz4Compressor.compress(
      body, 0, body.length, compressed, 0, maxCompressedLength);
