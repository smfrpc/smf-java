...
# That is slightly modified version of https://github.com/smfrpc/smf/blob/master/tools/fmt.py# - including :
# - clang-format in version 8.0.0
# - ignoring files generated by flatbuffers
...
def main():
    ...
    for f in files:
        ...
        insert_legal(f)
        try:
            if is_clang_fmt_file(f) and is_ignored_file(f):
                # fixing code
